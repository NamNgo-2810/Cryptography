...
class AES(object):
    ...
    ciphertext = aes.encryption('00112233445566778899aabbccddeeff', key)
    ...
    def SubWord(self, byte):
        ...
        :return: Substituted bytes through S-box
        """
    ...
    def SubBytes(self, state, isInv):
        ...
        return ['%02x' % self.s_box[int(state[x], 16)] for x in range(16)] if not isInv \
            else ['%02x' % self.inverted_s_box[int(state[x], 16)] for x in range(16)]
    ...
    def key_handler(self, key, isInv):
        ...
        return self.ExpandKey(key) if not isInv else [re.findall('.' * 2, self.RevertStateMatrix(x)) for x in
                                                      self.ExpandKey(key)]
